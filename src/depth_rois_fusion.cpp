...
private:
  std::string pointcloud_topic_, camera_info_topic_, rois_topic_, depth_image_topic_, depth_rois_image_topic_, out_camera_info_topic_;
  ...
  {
    //RCLCPP_WARN(get_logger(), "pointcloud_callback: %.3f", rclcpp::Time(msg->header.stamp).seconds());
    if (!camera_info_) {
    ...
    try {
      transform = tf_buffer_->lookupTransform(
        camera_info_->header.frame_id, msg->header.frame_id, msg->header.stamp, rclcpp::Duration::from_seconds(0.2));
    } catch (const std::exception & e) {
      RCLCPP_WARN(get_logger(), "Could not get transform: %s", e.what());
      return;
    }
    ...
    for (const auto & pt : points) {
      Eigen::Vector3d pt_cam = eigen_transform * pt;
    ...
    auto out_msg = cv_bridge::CvImage(msg->header, "32FC1", undistorted_depth_).toImageMsg();
  ...
  {
    //RCLCPP_INFO(get_logger(), "rois_callback: %.3f", rclcpp::Time(msg->header.stamp).seconds());
    ...
    //RCLCPP_INFO(get_logger(), "depth rois pub time");
    ...
    for (const auto & obj : msg->feature_objects) {
      ...
      //cv::Mat valid;
      //cv::threshold(roi_depth, valid, 0, 1, cv::THRESH_BINARY);
      double minVal, medianVal;
      //RCLCPP_INFO(get_logger(), "valid.type() = %d", valid.type());
      ...
      if (!valid_values.empty()) {
        ...
        medianVal = valid_values[valid_values.size() / 2];
      } else {
        medianVal = 0.0;
        minVal = 0.0;
      }
      ...
      char text[100];
      //snprintf(text, sizeof(text), "Med: %.2f m / Min: %.2f m", medianVal, minVal);
